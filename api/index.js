...
function keepalive() {
    // 1.请求主页，保持唤醒
    let vercel_app_url = `https://${VERCEL_URL}.vercel.app`;
    exec("curl " + vercel_app_url, function (err, stdout, stderr) { });

    // 2.请求服务器进程状态列表，若web没在运行，则调起
    exec("curl " + vercel_app_url + "/status", function (err, stdout, stderr) {
        if (!err) {
            if (stdout.indexOf("./app.js server --no-prefix") != -1) {
            } else {
                //web未运行，命令行调起
                exec("sh start.sh server");
            }
